...
cd /mnt/etc/nixos
cp hardware-configuration.nix "$FLAKE_ROOT"/hosts/elysium/hardware-configuration.nix && cp hardware-configuration.nix "$FLAKE_ROOT"/hosts/minimal/hardware-configuration.nix

relative_path="../../lib/disko_layout"
layout_filepath="$relative_path/$(basename $partition_layout)"

sed -i "/imports\ =/cimports\ = [(import\ $layout_filepath\ {})]++" "$FLAKE_ROOT"/hosts/{elysium,minimal}/hardware-configuration.nix
mkdir /mnt/etc/nixos/Flakes; cp -r "$FLAKE_ROOT/" /mnt/etc/nixos/Flakes/
lsblk
